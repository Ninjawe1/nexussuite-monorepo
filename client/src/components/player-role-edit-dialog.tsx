...
const roleEnumValues = TEAM_ROLES.map(r => r.value) as [typeof TEAM_ROLES[number]["value"], ...typeof TEAM_ROLES[number]["value"][]];

const schema = z.object({
  role: z.enum(roleEnumValues).default("Player"),
});

export function PlayerRoleEditDialog({ open, onOpenChange, player, rosterName, currentRole = "Player", onSubmit }: PlayerRoleEditDialogProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {player ? `Update ${player.name}'s role${rosterName ? ` for ${rosterName}` : ''}` : "Update player role"}
          </DialogDescription>
        </DialogHeader>
