...
export default function AdminContentPage() {
  ...
  const exportData = () => toast({ title: "Export", description: `Exported as ${exportFormat.toUpperCase()} (demo)` });
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" onClick={restore}>Restore DB</Button>
            <Input className="w-40" value={exportFormat} onChange={(e) => setExportFormat(e.target.value)} />
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[1,2,3,4,5,6].map(i => (
              <div key={i} className="aspect-square bg-muted rounded-lg flex items-center justify-center text-muted-foreground">Asset {i}</div>
            ))}
          </div>
        </CardContent>
      </Card>
