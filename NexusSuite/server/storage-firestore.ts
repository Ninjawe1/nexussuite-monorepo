...
async function createDoc<T>(collection: string, data: any): Promise<WithId<T>> {
  ...
  // Remove undefined values recursively because Firestore rejects them
  const cleanedPayload = JSON.parse(
    JSON.stringify(payload, (_k, v) => (v === undefined ? null : v))
  );
  try {
    await col(collection).doc(id).set(cleanedPayload);
  } catch (err: any) {
    // Provide rich diagnostics to help pinpoint Firestore serialization issues (e.g., undefined values)
    try {
      const safePayload = JSON.parse(JSON.stringify(cleanedPayload, (_k, v) => (v === undefined ? "<undefined>" : v)));
      console.error(`[firestore.createDoc] Failed to write`, { collection, id, errMessage: err?.message || String(err), errCode: err?.code, payload: safePayload });
    } catch (_) {
      console.error(`[firestore.createDoc] Failed to write (payload not serializable)`, { collection, id, err });
    }
    throw err;
  }
  return cleanedPayload as WithId<T>;
}
...
class FirestoreStorage implements IStorage {
  ...
  // OAuth status helpers (no persistence, derived from env configuration)
  // This method is used by /api/oauth/status to inform the frontend which platforms are configured.
  async getOAuthStatus(_tenantId: string, _userId: string): Promise<Record<string, { configured: boolean; name: string; note?: string }>> {
    ...
    const platforms = [
      { key: "instagram", name: "Instagram", clientId: env("INSTAGRAM_CLIENT_ID"), clientSecret: env("INSTAGRAM_CLIENT_SECRET") },
      { key: "facebook", name: "Facebook", clientId: env("FACEBOOK_CLIENT_ID"), clientSecret: env("FACEBOOK_CLIENT_SECRET") },
      { key: "twitter", name: "Twitter / X", clientId: env("TWITTER_CLIENT_ID"), clientSecret: env("TWITTER_CLIENT_SECRET") },
      { key: "tiktok", name: "TikTok", clientId: env("TIKTOK_CLIENT_ID"), clientSecret: env("TIKTOK_CLIENT_SECRET") },
      { key: "youtube", name: "YouTube", clientId: env("YOUTUBE_CLIENT_ID"), clientSecret: env("YOUTUBE_CLIENT_SECRET") },
      { key: "twitch", name: "Twitch", clientId: env("TWITCH_CLIENT_ID"), clientSecret: env("TWITCH_CLIENT_SECRET") },
    ];
    ...
    for (const p of platforms) {
      ...
      status[p.key] = {
        configured,
        name: p.name,
        note: configured ? undefined : `Missing ${p.key.toUpperCase()}_CLIENT_ID/_CLIENT_SECRET`,
      };
    }
    return status;
  }
