...
To ensure architectural consistency, roster operations now use the local Express API via the shared `apiRequest` utility, the same pattern used across staff, payroll, matches, and contracts. The client no longer calls Firestore directly for roster CRUD. Endpoints:
- `GET /api/rosters` — list rosters for current tenant
- `POST /api/rosters` — create roster
- `PATCH /api/rosters/:id` — update roster
- `DELETE /api/rosters/:id` — delete roster
- `GET /api/rosters/:id/players` — list players on a roster
- `POST /api/rosters/:id/players` — assign players to a roster
- `DELETE /api/rosters/:id/players/:playerId` — remove player from a roster

This change centralizes validation, applies audit logging, and keeps authentication/session handling consistent.