...
describe('RosterCreationDialog', () => {
  ...
  it('renders dialog when open', () => {
    ...
    expect(screen.getByText('Set up a new roster for your team. Fill in the details below to get started.')).toBeInTheDocument();
  });
  ...
  it('shows validation errors for required fields', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Roster name must be at least 3 characters')).toBeInTheDocument();
  ...
  it('validates roster name format', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Roster name can only contain letters, numbers, spaces, hyphens, and underscores')).toBeInTheDocument();
    });
  });
  ...
  it('validates max players range', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Maximum players must not exceed 10')).toBeInTheDocument();
    });
  });
  ...
  it('successfully submits form with valid data', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Description (Optional)'), 'Main competitive roster');
  ...
  it('renders roster type options correctly', () => {
    ...
    expect(screen.getByText('For tournaments and serious play')).toBeInTheDocument();
    expect(screen.getByText('Casual')).toBeInTheDocument();
    expect(screen.getByText('For fun and relaxed gaming')).toBeInTheDocument();
    expect(screen.getByText('Practice')).toBeInTheDocument();
    expect(screen.getByText('For training and skill development')).toBeInTheDocument();
  });
