...
export function PlayerRosterDialog({ open, onOpenChange, player, existingRosters }: PlayerRosterDialogProps) {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Error", description: error.message || "Failed to assign roster", variant: "destructive" });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({ title: "Error", description: error.message || "Failed to remove roster", variant: "destructive" });
    },
  });
      ...
      <DialogContent>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={createMutation.isPending}>Assign</Button>
          </div>
        </form>
            ...
            <div className="space-y-2">
              {existingRosters.map(r => (
                <div key={r.id} className="flex items-center justify-between p-2 rounded-lg border">
                  <span className="text-sm">{r.game} • {r.role}</span>
                  <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(r.id)} title="Remove">
