...
export default function Dashboard() {
  ...
  // Finance metrics inspired by the provided mock
  const totalRevenue = payroll
    .filter((p) => p.status === "paid")
    .reduce((sum, p) => sum + parseFloat(p.amount), 0);
  const totalSpent = payroll.reduce((sum, p) => sum + parseFloat(p.amount), 0);
  const totalImpressions = campaigns.reduce((sum, c) => sum + (c.reach ?? 0), 0);
  const avgConversion = (() => {
    const withEngagement = campaigns.filter((c) => typeof c.engagement === "number");
    if (withEngagement.length === 0) return 0;
    const pct = withEngagement.reduce((sum, c) => sum + Number(c.engagement || 0), 0) / withEngagement.length;
