...
export function RosterTab({ organizationId, currentUserId, allPlayers }: RosterTabProps) {
  ...
  const handleAssignPlayers = async (payload: { rosterId: string; playerIds: string[]; role: string; game: string }) => {
    try {
      // Ensure the roster has a 'game' set before assigning players
      const needsGame = !selectedRoster?.game || selectedRoster?.game === '' || selectedRoster?.game === 'Unknown Game';
      if (needsGame) {
        const gameToSet = payload.game?.trim();
        if (!gameToSet) {
          throw new Error("Roster is missing required 'game'. Please set the roster's game before assigning players.");
        }
        ...
        setSelectedRoster((prev: any) => prev ? { ...prev, game: gameToSet } : prev);
      }
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to assign players. Please try again.",
        variant: "destructive",
      });
      throw error;
    }
    ...
    <div className="space-y-6">
      ...
      {rosters.length === 0 ? (
        ...
        <RosterList
          ...
          onRosterEdit={(roster) => { setSelectedRoster(roster); setIsEditDialogOpen(true); }}
        ...
        <RosterEditDialog
          ...
          onSubmit={async (data) => {
            try {
              ...
              toast({ title: 'Roster updated', description: 'Your changes have been saved.' });
            } catch (e) {
              toast({ title: 'Failed to update roster', description: 'Please try again later.', variant: 'destructive' });
            }
          }}
          isLoading={isLoadingRosters}
        />
      ...
      {selectedRoster && (
        <PlayerAssignmentDialog
          ...
          currentPlayers={rosterPlayers.map(p => ({ playerId: p.playerId, role: p.role }))}
