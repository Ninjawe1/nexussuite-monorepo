...
import { PlayerAssignment } from '../services/rosterService';
...
describe('PlayerAssignmentDialog', () => {
  ...
  it('renders dialog when open', () => {
    ...
    expect(screen.getByText('Assign Players to Team Alpha')).toBeInTheDocument();
    expect(screen.getByText('Select players to add to this roster. You can add up to 5 more players.')).toBeInTheDocument();
  });
  ...
  it('prevents selecting more players than available slots', async () => {
    ...
    // Should show warning
    await waitFor(() => {
      expect(screen.getByText('You\'ve selected 2 players but only 1 slots are available.')).toBeInTheDocument();
    });
  });
  ...
  it('allows selecting role for assignment', async () => {
    ...
    const submitButton = screen.getByRole('button', { name: 'Assign 1 Players' });
  ...
  it('shows validation error when no players are selected', async () => {
    ...
    const submitButton = screen.getByRole('button', { name: 'Assign 0 Players' });
    fireEvent.click(submitButton);

    await waitFor(() => {
      expect(screen.getByText('Please select at least one player')).toBeInTheDocument();
    });
  });
