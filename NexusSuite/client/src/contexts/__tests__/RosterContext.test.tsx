...
jest.mock("../services/rosterService", () => ({
  RosterService: {
    ...
    assignPlayersToRoster: (...args: any[]) =>
      mockAssignPlayersToRoster(...args),
    removePlayerFromRoster: (...args: any[]) =>
      mockRemovePlayerFromRoster(...args),
  ...
  return (
    <div>
      <div data-testid="loading">
        {rostersLoading ? "Loading" : "Not Loading"}
      </div>
      ...
      <button
        onClick={() => updateRoster("roster-1", { name: "Updated Roster" })}
      >
        Update Roster
      </button>
      <button onClick={() => deleteRoster("roster-1")}>Delete Roster</button>
      <button
        onClick={() =>
          assignPlayersToRoster("roster-1", ["player-1", "player-2"])
        }
      >
        Assign Players
      </button>
...
describe("RosterContext", () => {
  ...
  it("handles roster creation", async () => {
    mockCreateRoster.mockResolvedValueOnce({
      id: "roster-new",
      name: "Test Roster",
    });
  ...
  it("handles roster update", async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateRoster).toHaveBeenCalledWith("roster-1", {
        name: "Updated Roster",
      });
    });
  });
  ...
  it("handles player assignment", async () => {
    ...
    await waitFor(() => {
      expect(mockAssignPlayersToRoster).toHaveBeenCalledWith("roster-1", [
        "player-1",
        "player-2",
      ]);
    });
  ...
  it("handles player removal", async () => {
    ...
    await waitFor(() => {
      expect(mockRemovePlayerFromRoster).toHaveBeenCalledWith(
        "roster-1",
        "player-1",
      );
    });
