...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      return (
        <div className={tweakcn("font-medium", labelClassName)}>{value}</div>
      );
    }, [
      label,
      labelFormatter,
...
// Unified dashboard chart wrappers
// These components standardize styling and usage across the app with shadcn/ui chart primitives.

type LineSeries = { key: string; label?: string; colorToken?: string };
type BarSeries = { key: string; label?: string; colorToken?: string };

function buildConfigFromSeries(series: Array<{ key: string; label?: string; colorToken?: string }>): ChartConfig {
  // Map series to chart color tokens in order: chart-1 ... chart-5
  const defaultTokens = [
    "hsl(var(--chart-1))",
    "hsl(var(--chart-2))",
    "hsl(var(--chart-3))",
...
export function DashboardLineChart({
  ...
  const config = React.useMemo(() => buildConfigFromSeries(series), [series]);
  return (
    <div className="h-[var(--chart-height)]" style={{ ['--chart-height' as any]: `${height}px` }}>
      <ChartContainer config={config} className="aspect-auto h-full">
        <RechartsPrimitive.LineChart data={data}>
          ...
          <RechartsPrimitive.XAxis dataKey={xKey} tick={{ fill: "hsl(var(--muted-foreground))" }} tickFormatter={xTickFormatter as any} />
          <RechartsPrimitive.YAxis tick={{ fill: "hsl(var(--muted-foreground))" }} tickFormatter={yTickFormatter as any} />
