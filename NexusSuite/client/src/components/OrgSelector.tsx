...
export function OrgSelector({ className = '' }: OrgSelectorProps) {
  ...
  if (isLoading) {
    return <div className={`h-[45px] w-full rounded-lg bg-muted/40 animate-pulse ${className}`} />;
  }
    ...
    return (
      <SidebarMenu>
        <SidebarMenuItem>
          <SidebarMenuButton
            size="default"
            className={`h-[45px] px-2 py-1.5 ${className}`}
            disabled
          >
                      ...
                      >
                        {isPending ? (
                          ...
                          <Avatar className="h-8 w-8 shrink-0 rounded-full">
                            <AvatarImage
                              src={(currentOrganization as any)?.avatarUrl || ''}
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter a name for your new organization. You will switch to it automatically after
              creation.
            </DialogDescription>
          </DialogHeader>
