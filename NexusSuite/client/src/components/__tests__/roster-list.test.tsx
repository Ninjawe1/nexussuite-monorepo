...
describe("RosterList", () => {
  ...
  it("renders roster list with correct data", () => {
    render(
      <RosterList rosters={mockRosters} isLoading={false} {...mockHandlers} />,
    );
  ...
  it("shows loading state", () => {
    render(<RosterList rosters={[]} isLoading={true} {...mockHandlers} />);

    expect(screen.getByText("Loading rosters...")).toBeInTheDocument();
  });

  it("shows empty state", () => {
    render(<RosterList rosters={[]} isLoading={false} {...mockHandlers} />);

    expect(screen.getByText("No rosters found")).toBeInTheDocument();
    expect(
      screen.getByText("Create your first roster to get started"),
    ).toBeInTheDocument();
  });
  ...
  it("disables actions when loading", () => {
    render(
      <RosterList rosters={mockRosters} isLoading={true} {...mockHandlers} />,
    );
