...
export default function Marcom() {
  ...
  const { data: socialAnalytics, isLoading: analyticsLoading } = useQuery<any>({
    ...
    queryFn: async () => {
      ...
      const res = await apiRequest(
        `/api/social/analytics?organizationId=${organization.id}`,
        'GET'
      );
      return res.json();
    },
  ...
  const deleteAccountMutation = useMutation({
    mutationFn: async (id: string) => {
      return await apiRequest(
        `/api/social/accounts/${id}?organizationId=${organization?.id}`,
        'DELETE'
      );
    },
    onSuccess: () => {
  ...
  const syncAccountMutation = useMutation({
    mutationFn: async (accountId: string) => {
      return await apiRequest(
        `/api/social/sync/${accountId}?organizationId=${organization?.id}`,
        'POST'
      );
    },
    onSuccess: () => {
  ...
  const avgEngagement =
    campaigns.filter(c => c.engagement).length > 0
      ? campaigns
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage campaigns and track social media performance
          </p>
        </div>
      </div>
          ...
          {accountsLoading ? (
            ...
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                No social accounts connected. Connect your first account to start tracking
                analytics.
              </CardContent>
            </Card>
          ) : (
                    ...
                    <CardContent className="space-y-2">
                      ...
                      {account.lastSyncedAt && (
                        <p className="text-xs text-muted-foreground">
                          Last synced: {formatDateSafe(account.lastSyncedAt, 'MMM dd, yyyy')}
                        </p>
                      )}
                    </CardContent>
                  </Card>
                ...
                {analyticsLoading ? (
                  ...
                  <>
                    <div
                      ...
                      data-testid="stat-total-followers"
                    >
                    ...
                    <Card key={index}>
                      ...
                      <CardContent className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-mono font-semibold">
                            {(platform.reach || 0).toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
            ...
            <Card>
              ...
              <CardContent>
                {campaignsLoading ? (
                  ...
                  <>
                    <div className="text-3xl font-bold font-mono">{avgEngagement.toFixed(1)}%</div>
      ...
      <SocialAccountOAuthDialog open={socialDialogOpen} onOpenChange={setSocialDialogOpen} />
    </div>
  );
}
