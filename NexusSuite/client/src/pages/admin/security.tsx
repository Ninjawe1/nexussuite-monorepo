...
export default function AdminSecurityPage() {
  ...
  const save = () =>
    toast({ title: "Security", description: "Settings saved (demo)" });
        ...
        <CardContent>
          ...
          <div className="mt-6 grid gap-3">
            ...
            <div className="grid gap-2">
              ...
              <Input value={target} onChange={(e) => setTarget(e.target.value)} placeholder={deliveryMethod === "email" ? "email" : "phone"} />
            </div>
            <div className="flex gap-2">
              <Button onClick={generateMfa} disabled={!target.trim()}>Send Code</Button>
            </div>
            <div className="grid gap-2">
      ...
      <Card>
        ...
        <CardContent>
          <ul className="space-y-2">
            {["alice@example.com failed", "bob@example.com success"].map(
              (l, idx) => (
                <li
                  key={idx}
